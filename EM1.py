...
def plot_results(X, y, n, title):
    ...
    plt.savefig("S-EM-Plot-" + str(n) + "-clust.png")
    plt.close()
    plt.figure()
...
def main():
    ...
    for i in N:
        ...
        r.append(adjusted_rand_score(labels, y))
        c.append(completeness_score(labels, y))
        h.append(homogeneity_score(labels,y))
    ...
    plt.savefig('S-EM-Silhouette.png')
    plt.close()
    plt.figure()

    n = 2
    plot_results(X, y, n, 'EM K='+str(n))
