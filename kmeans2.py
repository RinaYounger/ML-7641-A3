...
def plotit(X, y, n):
    ...
    plt.xticks(())
    plt.yticks(())
    plt.savefig("B-KM-" + str(n) + "clust.png")
...
def main():
    ...
    for i in N:
        ...
        h.append(homogeneity_score(train_y, kmeans.labels_))
        c.append((completeness_score(train_y, kmeans.labels_)))
        r.append(adjusted_rand_score(train_y, kmeans.labels_))
    ...
    plt.savefig('B-KM-Silhouette.png')
    plt.close()
    plt.figure()

    plotit(train_x,train_y,2)
