...
def plotem(X, y, n, name):
    ...
    plt.savefig("B-EM-"+name+"-"+str(n)+"clust.png")
    plt.close()
    plt.figure()
...
def plotkm(X, y, n, name):
    ...
    plt.xticks(())
    plt.yticks(())
    plt.savefig("B-KM-"+name+"-"+str(n)+"clust.png")
    plt.close()
    plt.figure()
...
def graphem(X):
    ...
    models = [PCA(random_state=42, n_components=20), FastICA(n_components=25, random_state=42),
              GaussianRandomProjection(n_components=35, random_state=42),
    ...
    plt.legend()
    plt.savefig('B-3-EM-Silhouette.png')
    plt.figure()
    plt.close()
    plt.figure()
...
def graphKM(X):
    ...
    models = [PCA(random_state=42, n_components=20), FastICA(n_components=25, random_state=42),  GaussianRandomProjection(n_components=35, random_state=42),
              FeatureAgglomeration(n_clusters=22)]
    names = ["PCA", "ICA", "GRP", "FA"]
    w = []
    c = []
    ...
    plt.figure()
    for x in range(len(s)):
        plt.plot(N, s[x], label=names[x])
    plt.title('Silhouette - K Means')
    plt.xlabel('Number of clusters')
    plt.ylabel('Silhouette Score')
    plt.legend()
    plt.savefig('B-3-KM-Silhouette.png')
    plt.figure()
    plt.close()
    plt.figure()
...
def main():
    ...
    plotkm(projected, train_y, 2, 'PCA')

    ica = FastICA(n_components=25, random_state=42)
    projected = ica.fit_transform(train_x)
    plotkm(projected, train_y, 3, 'ICA')

    grp = GaussianRandomProjection(n_components=35, random_state=42)
    projected = grp.fit_transform(train_x)
    plotkm(projected, train_y, 2, 'RP')

    fa = FeatureAgglomeration(n_clusters=22)
    projected = fa.fit_transform(train_x)
    plotkm(projected, train_y, 2, 'FA')
    ...
    plotem(projected, y, 2, 'PCA')


    ica = FastICA(n_components=25, random_state=42)
    projected = ica.fit_transform(X)
    plotem(projected, y, 10, 'ICA')
